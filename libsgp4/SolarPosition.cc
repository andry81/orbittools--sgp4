...
Eci SolarPosition::FindPosition(const DateTime& dt)
{
    ...
    const double year = 1900 + mjd / double(36525) * 1e2;
    const double T = (mjd + Delta_ET(year) / kSECONDS_PER_DAY) / 36525.0;
    const double M = Util::DegreesToRadians(Util::Wrap360(double(35847583) / 1e5
                + Util::Wrap360(double(int64_t(3599904975)) * T / 1e5)
                - (double(150) / 1e6 + double(33) * T / 1e7) * T * T));
    const double L = Util::DegreesToRadians(Util::Wrap360(double(27969668) / 1e5
                + Util::Wrap360(double(int64_t(3600076892)) * T / 1e5)
                + double(3025) * T * T / 1e7));
    const double e = double(1675104) / 1e8 - (double(418) / 1e7 + double(126) * T / 1e9) * T;
    const double C = Util::DegreesToRadians((double(1919460) / 1e6
    ...
    const double O = Util::DegreesToRadians(
            Util::Wrap360(double(25918) / 1e2 - double(1934142) * T / 1e3));
    ...
    double R = double(10000002) * (1 - e * e) / (1 + e * std::cos(nu)) / 1e7;
    const double eps = Util::DegreesToRadians(double(23452294) / 1e6 - (double(130125) / 1e7
...
{
    return double(26465) / 1e3 + double(747622) * (year - 1950) / 1e6 + double(1886913)
